...
const RegisterForm = ({setLogin}) => {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      const userData = {
        uid: user.uid, email: user.email, role: role, firstName: firstName, lastName: lastName
      }
      await dispatch(loggedIn(userData))
      if (isAuthenticated) {
  ...
  return (<Card className="mx-auto max-w-sm mt-20">
    <CardHeader>
